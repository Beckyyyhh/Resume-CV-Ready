...
import { resumeChecklist, resumeActionVerbs } from "@/lib/content";
    ...
    <div className="space-y-5 animate-fade-in">
      <div>
        ...
        <p className="mt-2 text-[15px] leading-relaxed text-gray-700">
          Look over your resume in the preview, tick off the checklist, then download your finished resume.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <div className="flex items-center justify-between mb-2.5">
          <p className="text-sm font-bold" style={{ color: "#3d2c8d" }}>
            Resume final checklist
          </p>
          <span className="text-xs font-semibold text-gray-400">
            {doneCount}/{resumeChecklist.length}
          </span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <p className="text-sm font-bold mb-2" style={{ color: "#3d2c8d" }}>
          Strong action verbs to start your dot points
        </p>
        <div className="flex flex-wrap gap-1.5">
          {resumeActionVerbs.map((v) => (
            <span
              key={v}
              className="text-xs font-semibold px-2.5 py-1 rounded-full"
              style={{ backgroundColor: "#f3f1fc", color: "#534ab7" }}
            >
              {v}
            </span>
          ))}
